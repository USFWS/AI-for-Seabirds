...
export_csv = "C:/BP/Annotation_analysis/new_character_Mar10.csv"

total_pixels = 6464 * 4848
...
for file in files:
    ...
    # Texture analysis
    glcm = graycomatrix(img_grey, distances=[10], angles=[45], levels=256, symmetric=True, normed=True)

    # Normalizes data
    contrast10 = graycoprops(glcm, 'contrast')
    contrast10_list.append(contrast10)
   # print("Local contrast: ", contrast10)

pd.DataFrame({"unique_image": image_list, "contrast_sd": contrast_sd_list, "percent_saturation": saturation_percent_list,
            "contrast": contrast10_list}).to_csv(export_csv, index=True)
