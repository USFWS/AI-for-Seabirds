...
root_path = "D:/flights_2025/"
flight_name = "20250211_095700"

model_path = "D:/MODELS FOR USE/2024_Nov20_swin_s_augment15_focal_scripted.pt"
...
detection_results = root_path + flight_name + "/" + flight_name + "_detections.csv"
detection_results  = pandas.read_csv(detection_results)
image_dir = root_path + flight_name + "/crops_inference_birds/"
...
# classify_csv = path/name of csv file of results to export
...
classify_csv = root_path + flight_name + "/classify_" + flight_name + ".csv"
...
for index, row in matches.iterrows():  ## iterrows: Pandas iterate over rows
    ...
    xmin_box = row['xmin'] - 17
    if xmin_box < 0:
        xmin_box = 0
    ymin_box = row['ymin'] - 17
    if ymin_box < 0:
        ymin_box = 0
    xmax_box = row['xmin'] + row['w'] + 17
    ymax_box = row['ymin'] + row['h'] + 17
...
classify_results = pandas.read_csv(classify_csv)
...
for index, row in classify_results.iterrows():
    ...
    if score1 <  max_prob_threshold and score1 > min_prob_threshold:
        source_to_move = image_context + row['unique_image_jpg']  # +'.jpg'
        print('source_to_move : ', source_to_move)
        img_name = os.path.basename(source_to_move)
        dir2 = root_export + row['label1']
        if not os.path.exists(dir2):
            os.makedirs(dir2)
        dest = root_export + row['label1'] + '/' + img_name
        print("Destination : ", dest)
        shutil.copy(source_to_move, dest)  # this can be changed to: shutil.move
