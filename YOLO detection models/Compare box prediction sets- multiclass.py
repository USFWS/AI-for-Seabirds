...
against every SAME-CLASS box in file B, and overlap metrics are computed.
"""
import pandas as pd
import config
...
def compare_two_prediction_sets(csv_path_1, csv_path_2, only_overlaps=False):
    ...
    Every box in set 1 is only ever compared against SAME-CLASS boxes in set 2,
    for each image. Boxes of a different class_id are never considered
    candidates, so a "no match" for one class can't be silently filled in by
    an overlapping box of a different class.

    Every box in set 1 is guaranteed to appear at least once in the output,
    even if set 2 has no same-class boxes on that image, or none overlap it.
    ...
    for (fname, class_id), entries_1 in boxes_1.items():
        entries_2 = _entries_for(boxes_2, fname, class_id)  # same class only
...
def best_match_each_direction(csv_path_1, csv_path_2):
    ...
    Find the best (highest IoU) match for every box, in both directions:
      - best_from_1: for each box in set 1, its best SAME-CLASS match in set 2
      - best_from_2: for each box in set 2, its best SAME-CLASS match in set 1
    ...
    iou = 0.0):
      1. There were no same-class candidate boxes at all on that image.
      2. There were same-class candidates, but none of them overlapped
         (best IoU = 0).
    """
    ...
    if best_from_2_raw.empty:
        ...
        best_from_2 = pd.DataFrame({
            ...
            "class_id": best_from_2_raw["class_id"],
...
def _self_test_unmatched_boxes():
    """
    Sanity check covering two things:
      1. Boxes with NO real match -- whether because there were no same-class
         candidates at all, or because the best same-class candidate had zero
         overlap -- are both normalized to a missing set2_row/set1_row.
      2. Boxes are NEVER matched across different class_id values, even when
         a wrong-class box would otherwise be a perfect geometric overlap.

    Uses the CURRENT configured column names (FILENAME_COL, X_COL, Y_COL,
    W_COL, H_COL, CLASS_COL) so it stays valid even after you rename columns
    in config.
    """
    import tempfile, os
    ...
    csv1_content = (
        f"{header}\n"
        "img1.jpg,0,0,10,10,0\n"     # class 0, should match set2 row below
        "img1.jpg,100,100,10,10,0\n" # class 0, no nearby match -> no match
        "img2.jpg,5,5,10,10,0\n"     # class 0, no counterpart image in set2 at all
        "img4.jpg,0,0,10,10,1\n"     # class 1, perfectly overlaps a class-0 box in set2 -- must NOT match
    )

    csv2_content = (
        f"{header}\n"
        "img1.jpg,1,1,10,10,0\n"     # class 0, should match set1 row above
        "img3.jpg,0,0,10,10,0\n"     # class 0, no counterpart image in set1 at all
        "img4.jpg,0,0,10,10,0\n"     # class 0, same geometry as img4 above but WRONG class
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # set1 has 4 boxes total -> best_from_1 must have 4 rows
        assert len(best_from_1) == 4, f"Expected 4 rows in best_from_1, got {len(best_from_1)}"

        # set2 has 3 boxes total -> best_from_2 must have 3 rows
        assert len(best_from_2) == 3, f"Expected 3 rows in best_from_2, got {len(best_from_2)}"

        # The img1.jpg class-0 box at (0,0) should find its class-0 match in set2
        real_match_row = best_from_1[
            (best_from_1["filename"] == "img1.jpg") & (best_from_1["set1_row"] == 0)
        ]
        assert not pd.isna(real_match_row.iloc[0]["set2_row"]), "Expected the class-0 img1 box to find its match"
        assert real_match_row.iloc[0]["iou"] > 0, "Expected a positive IoU for the real match"

        # The img1.jpg box at (100,100) has no nearby match -> should show set2_row missing
        ...
        # THE KEY CLASS TEST: img4.jpg boxes perfectly overlap geometrically but
        # have different class_id (1 in set1, 0 in set2) -- they must NOT match.
        class_mismatch_row = best_from_1[best_from_1["filename"] == "img4.jpg"]
        assert pd.isna(class_mismatch_row.iloc[0]["set2_row"]), (
            "Class filtering failed: a class-1 box matched a class-0 box just because "
            "they overlap geometrically."
        )
        class_mismatch_row_2 = best_from_2[best_from_2["filename"] == "img4.jpg"]
        assert pd.isna(class_mismatch_row_2.iloc[0]["set1_row"]), (
            "Class filtering failed in reverse direction: a class-0 box matched a "
            "class-1 box just because they overlap geometrically."
        )

        print("All unmatched-box and class-filtering checks passed.")
        print("\nbest_from_1:")
